...
- [x] T306 [US3] Implement optimizer driver `src/optimizer/evolutionary.py` (multiprocessing evaluation, weighted objective, gates)
- [x] T307 [US3] Implement telemetry publisher `src/optimizer/telemetry.py` integrating with `TrainingLogger`
- [x] T308 [US3] Create Streamlit page `pages/2_Model_Builder.py` with parameter controls, bounds, telemetry display, save genome actions
- [x] T309 [US3] Add CLI command `optimize` mirroring UI flow with JSON output and console summary
- [x] T310 [US3] Persist best genomes via `storage/artifacts.py` and update portfolio provenance if needed
